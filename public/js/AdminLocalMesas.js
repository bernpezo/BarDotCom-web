$(document).ready( function () {
     ...
     * Data table
     */
    var tabla=$('#tablaMesas').DataTable({
		//idioma de datatable
		"bProcessing": false,
		"serverSide": true,
		"width" : "100%",
    ...
    function eliminarMesa() {
    	$('.btn-eliminar').click(function(event) {
			swal.fire({
			  title: '¿Está seguro?',
			  text: '¿Desea eliminar la mesa: '+$(this).data("ing")+'?',
			  type: 'warning',
			  ...
			  confirmButtonText: 'Si, eliminar!'
				...
				if (result.value) {
					destroyMesa($(this).data('url'),$(this).data('id'));
				}
			})
		});
    }
    function destroyMesa(url, id) {
		$.ajax({                        
			type: 'post',                 
			url: url,                     
			data: {id:id},
