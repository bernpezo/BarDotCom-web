$(document).ready( function () {
     ...
     * Data table
     */
    var tabla=$('#tablaItems').DataTable({
		//idioma de datatable
		"bProcessing": false,
		"serverSide": true,
		"width" : "100%",
    ...
    function eliminarItem() {
    	$('.btn-eliminar').click(function(event) {
			swal.fire({
			  title: '¿Está seguro?',
			  text: '¿Desea eliminar el ítem: '+$(this).data("ing")+'?',
			  type: 'warning',
			  ...
			  confirmButtonText: 'Si, eliminar!'
				...
				if (result.value) {
					destroyItem($(this).data('url'),$(this).data('id'));
				}
			})
		});
    }
    function destroyItem(url, id) {
		$.ajax({                        
			type: 'post',                 
			url: url,                     
			data: {id:id},
