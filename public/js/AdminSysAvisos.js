$(document).ready( function () {
     ...
     * Data table
     */
    var tabla=$('#tablaAvisos').DataTable({
		//idioma de datatable
		"bProcessing": false,
		"serverSide": true,
		"width" : "100%",
    ...
    function eliminarAviso() {
    	$('.btn-eliminar').click(function(event) {
			swal.fire({
			  title: '¿Está seguro?',
			  text: '¿Desea eliminar el aviso: '+$(this).data("ing")+'?',
			  type: 'warning',
			  ...
			  confirmButtonText: 'Si, eliminar!'
				...
				if (result.value) {
					destroyAviso($(this).data('url'),$(this).data('id'));
				}
			})
		});
    }
    function destroyAviso(url, id) {
		$.ajax({                        
			type: 'post',                 
			url: url,                     
			data: {id:id},
